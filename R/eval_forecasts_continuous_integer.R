...
eval_forecasts_sample <- function(data,
  ...
  if ("sharpness" %in% metrics) {
    data[, sharpness := scoringutils2::sharpness(t(prediction)), by = c(by)]
  }
  # bias
  if ("bias" %in% metrics) {
    data[, bias := scoringutils2::bias(unique(true_value),
                                      t(prediction)), by = c(by)]
  }
  # DSS
  if ("dss" %in% metrics) {
    data[, dss := scoringutils2::dss(unique(true_value),
                                    t(prediction)), by = c(by)]
  }
  # CRPS
  if ("crps" %in% metrics) {
    data[, crps := scoringutils2::crps(unique(true_value),
                                      t(prediction)), by = c(by)]
  }
  # Log Score
  if ("log_score" %in% metrics) {
    ...
    if (prediction_type == "continuous") {
      data[, log_score := scoringutils2::logs(unique(true_value),
                                             t(prediction)), by = c(by)]
    }
  }
  # coverage
